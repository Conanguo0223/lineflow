...
class LineSimulation(gym.Env):
    ...
    def __init__(self, line, simulation_end, reward="parts", part_reward_lookback=0, render_mode=None):
        ...
        assert reward in ["uptime", "parts","test"]
        self.reward = reward
    ...
    def step(self, actions):
        ...
        elif self.reward == "test":
            part_component = (self.line.get_n_parts_produced() - self.n_parts) - \
                self.line.scrap_factor*(self.line.get_n_scrap_parts() - self.n_scrap_parts)
            uptime_component = self.line.get_uptime(lookback=10).mean()
            reward = part_component + 0.5*uptime_component
        else:
